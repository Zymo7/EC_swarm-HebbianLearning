...
def main():
    ...
    run_start = 0
    for arena in arenas:
        experiment_name = f"{arena}x{arena}_pop{pop_size}_dynamic_target"
        arena_type = f"circle_{arena}x{arena}"
        simulator_settings = EnvSettings
        simulator_settings['arena_type'] = arena_type
        simulator_settings['objectives'] = ['target_tracking']  # Changed from 'gradient' to 'target_tracking'
        simulator_settings['dynamic_target'] = True  # Enable dynamic target
        simulator_settings['target_radius'] = arena / 3  # Set radius to 1/3 of arena size
        simulator_settings['target_angular_velocity'] = 0.1  # Set angular velocity

        for run in range(run_start, run_start + n_runs):
            gen_start = 0
            genomes = []
            fitnesses = []
            experiment_dir = os.path.join("./results",'DynamicTarget', experiment_name, str(run))
...
if __name__ == '__main__':
    print("STARTING evolutionary experiment for Dynamic Target Tracking")
    main()
    print("FINISHED")
